...
class DropDownPicker extends React.Component {
    ...
    render() {
        ...
        const alignTextStyle = this.props.alignText == 'left' ? {justifyContent: 'flex-start'} : {};
        return (
            <View style={[this.props.containerStyle, {
                ...
                <TouchableOpacity onLayout={(event) => { this.getLayout(event.nativeEvent.layout) }} 
                    disabled={disabled} 
                    onPress={() => this.toggle()} activeOpacity={1} 
                    style={[styles.dropDown, this.props.style, this.state.visible && styles.noBottomRadius, {flexDirection: 'row', flex: 1}]}>
                    <View style={[styles.dropDownDisplay, alignTextStyle, this.props.dropDownDisplayStyle]}>
                    ...
                    <ScrollView style={{width: '100%', zIndex: this.props.zIndex}} nestedScrollEnabled={true}>
                    {
                        this.props.items.map((item, index) => (
                            <TouchableOpacity key={index} onPress={() => this.select(item, index)} style={[styles.dropDownItem, this.props.itemStyle, (
                                ...
                                <Text style={[this.props.labelStyle, this.props.itemLabelStyle,
